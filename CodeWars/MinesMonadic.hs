...
data Dungeon = Dungeon {mines :: [[Bool]], exit :: XY} deriving Show

type DungeonState = State Dungeon

assertPosition :: XY -> DungeonState Bool
assertPosition pos = do
    True <- inBounds pos
    True <- notBomb pos
    return True

inBounds :: XY -> DungeonState Bool
inBounds (x,y) = do
    (Dungeon mines _) <- get
    return $ not $ x < 0 || y < 0 || x >= length mines || y >= length (head mines)

notBomb :: XY -> DungeonState Bool
notBomb (x, y) = do
    (Dungeon mines _) <- get
    return $ (mines !! x) !! y
